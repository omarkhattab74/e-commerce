            ...
            {data?.map((order: singleOrder) =>
                <div key={order._id} className=' shadow-xl border-b-5 border-green-500 my-10'>
                    <div className='flex justify-between my-7 bg-white items-center p-6 rounded'>
                        <div>
                            <p className='font-bold md:text-[16px]  text-[10px]'>Order ID :</p>
                            <span className='text-[10px] md:text-[16px] '>{order.id}</span>
                        </div>
                        <div>
                            <p className='font-bold md:text-[16px]  text-[10px]'>Created At :</p>
                            <span className='text-[10px] md:text-[16px] '>{order.createdAt.slice(0, 10)}</span>
                        </div>
                        <div className='text-center'>
                            <p className='font-bold text-[10px] md:text-[16px]  mb-2'>Status :</p>
                            <span className='me-1 bg-orange-200 md:text-[16px]  text-[10px] p-1.5 rounded'>{order.isPaid ? "paid" : "Unpaid"}</span>
                            <span className=' bg-blue-200 p-1.5  md:text-[16px] text-[10px] rounded'> {order.isDelivered ? "Done" : "Proccessing"}</span>
                        </div>
                    </div>
                    ...
                    {order.cartItems.map((item) =>
                        <div key={item._id} className='flex justify-between my-7 p-7'>
                            <div className="left flex gap-5 w-[70%] flex-wrap">
                                ...
                                <div>
                                    <h2 className='text-[10px] md:text-[16px]'>{item.product.title}</h2>
                                    <p className='text-[10px] md:text-[16px]'>{item.product.brand.name}</p>
                                    <p className='text-[10px] md:text-[16px]'>{item.product.category.name}</p>
                                    <p className='text-[10px] md:text-[16px]'>{item.count}</p>

                                </div>
                            </div>
                            <div className='w-[25%] text-end'>
                                <p className='text-[10px] md:text-[16px]'>{item.price} Egp</p>
                            </div>
                        </div>
                    )}
